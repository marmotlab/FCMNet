...
from step_policy import StepPolicy
...
class Model(object):
    ...
    def __init__(self, env):
        ...
        with tf.variable_scope('ppo_model', reuse=tf.AUTO_REUSE):
            act_model = StepPolicy(env, N_ENVS, self.sess)
            self.train_model = train_model = TrainPolicy(env, MINIBATCH_SIZE, self.sess)
            evalue_model = TrainPolicy(env, 1, self.sess)
